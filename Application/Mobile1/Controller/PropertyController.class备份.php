...
class PropertyController extends CommonController
{
    ...
    public function claimDetail()
    {
        ...
        foreach ($propertyArr as $key=>$val)
        {
            ...
            //$w['batchnum'] = $batchnum;
            $temp_num = M('propertytake')->group('batchnum')->where($w)->select();
            $brought = M('propertytake')->where($w)->sum('nums');
            if (count($temp_num) < $goodsArr['getmaxnum']&&$brought<$val['totaltree']) {
                $batchnum[$key]['batchnum'] = $val['batchnum'];
                $batchnum[$key]['id'] = $val['id'];
            }
        }
    ...
    public function doClaim()
    {
        ...
        $addDate['order_status'] = $order_status;
        $addDate['pay_status'] = $pay_status;
        ...
        if ($total_price==0&&$res&&$res1) {
            ...
            if ($propertytake&&$trans) {
                M()->commit();
                echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
                echo "<script type='text/javascript'>alert('下单成功');window.location.href='".U('Orderlist/index')."';</script>";
                exit;
            }else{
                M()->rollback();
                echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
