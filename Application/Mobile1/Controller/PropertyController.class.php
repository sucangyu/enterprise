...
class PropertyController extends CommonController
{
    ...
    public function sendProperty()
    {
        ...
        if(IS_POST)
        {
            ...
            $addData2['memo'] = $_SESSION[$pre_session.'user']['nickname'].'给您赠送了'.$sendNum.'棵'.date('y',$propertyArr['batchnum']).'年的葡萄树';
    ...
    public function claimDetail()
    {
        ...
        $where['mid'] = $this->mid;
        $where['proj_id'] = $goodsArr['proj_id'];
        $where['batchnum'] = array('egt',time()+C('MIN_YEAR'));
        $propertyArr = M('Property')->where($where)->select();

        if(empty($propertyArr))
        {
            echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
            echo "<script type='text/javascript'>alert('您目前还没有可以领取当前收成的葡萄树');window.location.href='".U('Property/claimList')."';</script>";
            exit;
        }
    ...
    public function doClaim()
    {
        ...
        $setbatchnum = strtotime(date('Y-01-01 0:0:0',time()));
...
//        $addDate['batchnum'] = $goodsArr['batchnum'];
        ...
        // var_dump($addDate);
        // die;
        if(!$res = $orderlistM->add($addDate))
        {
            M()->rollback();
            echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
            echo "<script type='text/javascript'>alert('收成领取失败，错误代码00110');window.location.href='".U('Property/claimlist')."';</script>";
            exit;
        }

        $w['mid'] = $this->mid;
        $w['batchnum'] = $setbatchnum;
//        $w['batchnum'] = $goodsArr['batchnum'];
        $percentage = 0;//年份的待领取资源数
        $ogDate = array();
        for ($a=0; $a < count($_POST['years']); $a++) {
            $propertyArr = $propertyM->find($_POST['years'][$a]);
            $w['property_id'] = $_POST['years'][$a];
            $temp_num = $propertytakeM->where($w)->sum('nums');//已领取的资源数量
            if(empty($temp_num))
            {
                $temp_num = 0;
            }
            if(empty($propertyArr)||($temp_num!=0&&$temp_num>=$propertyArr['totaltree']))
            {
                continue;
            }

            $bro = $percentage+$propertyArr['totaltree']-$temp_num;//可领资源数==年份的待领取资源数+本年资源数-本年资源已领数
            $brought = floor($bro/$goodsArr['minpropertynum']);//本年份可领收成数
            $percentage = ($propertyArr['totaltree']-$temp_num)%$goodsArr['minpropertynum'];//本年可领的剩余资源数
            ...
            if($nums<=$brought)
            {
                $ogDate[$a]['goods_num'] = $nums;
                $ogDate[$a]['needtreenum'] = $goodsArr['minpropertynum']*$nums;
                break;
            }else{
                $ogDate[$a]['goods_num'] = $brought;
                $ogDate[$a]['needtreenum'] = $goodsArr['minpropertynum']*$brought;
                $nums -= $brought;
                continue;
            }
        }
        if(empty($ogDate))
        {
            M()->rollback();
            echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
            echo "<script type='text/javascript'>alert('收成领取失败，错误代码00111');window.location.href='".U('Property/claimlist')."';</script>";
            exit;
        }
        if(!$res1 = $ordergoodsM->addAll($ogDate))
        {
            M()->rollback();
            echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
            echo "<script type='text/javascript'>alert('收成领取失败，错误代码00112');window.location.href='".U('Property/claimlist')."';</script>";
            exit;
        }
        //到付或自提且加工费为0
        if ($total_price==0)
        {
            ...
            if(!$orderlistM->where('id='.$res)->save(array('order_status' => $order_status,'paykind' => 0,'pay_status' => 1,'pay_time' => time())))
            {
                M()->rollback();
                echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
                echo "<script type='text/javascript'>alert('收成领取失败，错误代码00113');window.location.href='".U('Property/claimlist')."';</script>";
                exit;
            }
            //添加资产收成表
            ...
            for ($i=0; $i < count($ogArr); $i++)
            {
                ...
                if(empty($propertyArr))
                {
                    M()->rollback();
                    echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
                    echo "<script type='text/javascript'>alert('收成领取失败，错误代码00114');window.location.href='".U('Property/claimlist')."';</script>";
                    exit;
                }
                ...
                if(!$propertytakeM->add($protakeData))
                {
                    M()->rollback();
                    echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
                    echo "<script type='text/javascript'>alert('收成领取失败，错误代码00115');window.location.href='".U('Property/claimlist')."';</script>";
                    exit;
                }
                unset($protakeData);
            }
